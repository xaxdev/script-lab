...
export class JupyterNotebook {
    ...
    private connect(): Promise<void> {
        ...
        .then((returnData) => {
            ...
            wsUrl = Util.combineUrl(wsUrl, 'api/kernels/' + this.m_kernelId + "/channels?token=" + this.m_conn.token + '&session_id=' + this.m_channelSessionId);
    ...
    private buildMessage(channel: string, msgType: string, content: any, metadata: any): JupyterWebSocketMessage {
        let msgId = Util.uuid();
        var msg = {
            header: {
                msg_id: msgId,
    ...
    private handleInputRequest(msg: JupyterWebSocketMessage) {
        ...
        if (typeof(content.prompt) === 'string' && content.prompt.substr(0, officeApiPrefix.length) === officeApiPrefix) {
            ...
            OfficeExtension.HttpUtility.sendLocalDocumentRequest(requestInfo)
                .then((responseInfo: OfficeExtension.HttpResponseInfo) => {
                    let responseInfoStr = JSON.stringify(responseInfo);
                    let respMsg = this.buildMessage('stdin', 'input_reply', {value: responseInfoStr}, null);
